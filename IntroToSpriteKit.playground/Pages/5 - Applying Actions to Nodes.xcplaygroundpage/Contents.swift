...
let actionReallyLongWaitThenMoveRight = SKAction.sequence([actionTwentySecondWait, actionRightMovement])

// Now run the sequence on the first circle
circle1.run(actionReallyLongWaitThenMoveRight)
// Exercise 2: Write your code below.
let leftAndUpThisMuch = CGVector(dx: 250, dy: 250)

let actionReallyLongWaitThenMoveUpAndLeft = SKAction.sequence([actionTwentySecondWait, actionRightMovement])
